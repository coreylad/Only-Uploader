...
if sys.version_info < (3, 12):
    print('Python 3.12+ is required (got %s)' % sys.version)
    sys.exit(1)
" || error "Please upgrade Python to 3.12 or higher."
...
RTSERVICE
    ...
    echo "      \"rtorrent_url\": \"http://127.0.0.1/RPC2\","
...
NGINXCONF
        ...
        # ruTorrent config: point it at the rtorrent RPC socket
...
$scgi_port = 0;
$scgi_host = "unix:///RPC2";  // Nginx proxies /RPC2 -> rtorrent socket
$XMLRPCMountPoint = "/RPC2";
RUCONF
            sudo chown "${WEB_USER}:${WEB_USER}" "${RUTORRENT_CONFIG_PHP}"
            success "ruTorrent config written."
        else
